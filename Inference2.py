...
prompt_template = (
    "Please rewrite the following complex sentence to make it easier for individuals with dyslexia. "
    "Focus on using shorter words, breaking long sentences into shorter ones, and providing clear, direct language. "
    "Replace any difficult words with simpler synonyms. Avoid using jargon, technical terms, and idiomatic expressions. "
    "Make sure the simplified sentence is easy to read and understand. The final simplified sentence should be "
    "grammatical, fluent, and retain the main ideas without changing the meaning.\n\n"
    f"{few_shot_examples}"
...
with open("output_data_flan_UL2.json", "w") as outfile:
    json.dump(results, outfile, indent=4)

# Prepare data for metric calculation
source_sentences = [result['complex'] for result in results]
predictions = [result['prediction'] for result in results]
references = [result['simple'] for result in results]

# Ensure references are correctly formatted for SARI
formatted_references = list(map(list, zip(*references)))

# Calculate SARI score
sari_scores = []
for src, pred, refs in zip(source_sentences, predictions, formatted_references):
    sari_add, sari_keep, sari_del = get_corpus_sari_operation_scores(
        [src],
        [pred],
        [refs]  # Already formatted as (n_samples, n_references)
    )
    sari_score = (sari_add + sari_keep + sari_del) / 3
    sari_scores.append(sari_score)
average_sari_score = sum(sari_scores) / len(sari_scores)
